...
###exit statuses
laststatus=2 # passed as parameter
playexitcode=2
shuffleexitcode=3
quitexitcode=1
autoplayexitcode=20
autoshuffleexitcode=30


###autoclose

maxcycles=200 # this is max of idle cycles the TUI can do before closing itself out
autoclosecycles=0 # the amount of cycles 
...
function main_tui(){
	...
	# getting last item offset
	lastitemoffset=$1
	itemoffset=$lastitemoffset

	# getting last exit status
	laststatus=$2
	...
	while true;do
		...
		# update the amount of cycles 
		autoclosecycles=$(( autoclosecycles + 1 ))

		if [ $autoclosecycles -gt $maxcycles ];then
			itemoffset=$(( lastitemoffset + 1 ))
			closegracifully $autoplayexitcode
		fi
